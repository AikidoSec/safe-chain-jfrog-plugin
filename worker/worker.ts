...
import {
    ...
    DOWNLOAD_OK_NOT_REMOTE, DOWNLOAD_OK_SAFE,
    DOWNLOAD_STOP_ERROR, DOWNLOAD_STOP_MALWARE,
...
export default async (context: PlatformContext, data: BeforeDownloadRequest): Promise<BeforeDownloadResponse> => {
    try {
        ...
        console.log(`safe-chain scanning package ${packageDetails.name} and version ${packageDetails.version}`);
        ...
        if (malwareDatabase.isMalware(packageDetails.name, packageDetails.version)) {
